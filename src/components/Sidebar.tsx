...
const SidebarItem = ({ icon: Icon, label, to, subItems = false }: { icon: any, label: string, to: string, subItems?: boolean }) => (
  <NavLink 
    to={to}
  ...
  >
    ...
    {subItems && <ChevronRight size={14} className="text-zinc-600" />}
  </NavLink>
);
    ...
    <aside className="hidden md:flex w-64 border-r border-zinc-200 dark:border-zinc-800 flex-col h-screen sticky top-0 bg-white dark:bg-[#1C1C1C] z-40">
      ...
      <div className="px-4 space-y-6 flex-1 overflow-y-auto pb-10">
        ...
        <div>
          ...
          <SidebarItem icon={TrendingUp} label="Analytics" to={`${baseUrl}/analytics`} subItems />
          <SidebarItem icon={Users} label="War Room" to={`${baseUrl}/war-room`} subItems />
          <SidebarItem icon={Target} label="North Star" to={`${baseUrl}/north-star`} subItems />
        </div>

        <div>
          <p className="px-3 text-[10px] font-bold text-zinc-600 uppercase tracking-widest mb-2">Pages</p>
          <SidebarItem icon={UserCircle} label="User Profile" to={`${baseUrl}/profile`} subItems />
          <SidebarItem icon={FileText} label="Documents" to={`${baseUrl}/documents`} subItems />
          <SidebarItem icon={MessageSquare} label="Social" to={`${baseUrl}/social`} subItems />
        </div>
      </div>
